...
-- RLS Performance Optimization: Phase 1 - Create Optimized Security Functions (Corrected)
...
-- Step 2: Drop all existing problematic RLS policies (with proper error handling)
DO $$ 
DECLARE 
    r RECORD;
...
BEGIN
    ...
    FOR r IN (
        ...
        AND tablename IN (
            ...
            'processing_telemetry', 'job_prompt_usage', 'prompt_templates',
            'question_flows', 'resume_entity_links', 'auth_rate_limits', 'security_audit_log'
        )
    ) LOOP
        BEGIN
...
BEGIN
    -- Career enrichment
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'career_enrichment' AND policyname = 'optimized_career_enrichment_access') THEN
        CREATE POLICY "optimized_career_enrichment_access" ON public.career_enrichment
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Career narratives
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'career_narratives' AND policyname = 'optimized_career_narratives_access') THEN
        CREATE POLICY "optimized_career_narratives_access" ON public.career_narratives
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Career profile
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'career_profile' AND policyname = 'optimized_career_profile_access') THEN
        CREATE POLICY "optimized_career_profile_access" ON public.career_profile
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Certification
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'certification' AND policyname = 'optimized_certification_access') THEN
        CREATE POLICY "optimized_certification_access" ON public.certification
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Education
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'education' AND policyname = 'optimized_education_access') THEN
        CREATE POLICY "optimized_education_access" ON public.education
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Entry enrichment
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'entry_enrichment' AND policyname = 'optimized_entry_enrichment_access') THEN
        CREATE POLICY "optimized_entry_enrichment_access" ON public.entry_enrichment
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Interview sessions
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'interview_sessions' AND policyname = 'optimized_interview_sessions_access') THEN
        CREATE POLICY "optimized_interview_sessions_access" ON public.interview_sessions
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Interviews
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'interviews' AND policyname = 'optimized_interviews_access') THEN
        CREATE POLICY "optimized_interviews_access" ON public.interviews
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Jobs
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'jobs' AND policyname = 'optimized_jobs_access') THEN
        CREATE POLICY "optimized_jobs_access" ON public.jobs
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Merge decisions
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'merge_decisions' AND policyname = 'optimized_merge_decisions_access') THEN
        CREATE POLICY "optimized_merge_decisions_access" ON public.merge_decisions
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Project
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'project' AND policyname = 'optimized_project_access') THEN
        CREATE POLICY "optimized_project_access" ON public.project
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Resume streams
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'resume_streams' AND policyname = 'optimized_resume_streams_access') THEN
        CREATE POLICY "optimized_resume_streams_access" ON public.resume_streams
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Resume uploads
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'resume_uploads' AND policyname = 'optimized_resume_uploads_access') THEN
        CREATE POLICY "optimized_resume_uploads_access" ON public.resume_uploads
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Skill
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'skill' AND policyname = 'optimized_skill_access') THEN
        CREATE POLICY "optimized_skill_access" ON public.skill
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- User confirmed profile
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'user_confirmed_profile' AND policyname = 'optimized_user_confirmed_profile_access') THEN
        CREATE POLICY "optimized_user_confirmed_profile_access" ON public.user_confirmed_profile
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Work experience
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'work_experience' AND policyname = 'optimized_work_experience_access') THEN
        CREATE POLICY "optimized_work_experience_access" ON public.work_experience
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Enrichment jobs
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'enrichment_jobs' AND policyname = 'optimized_enrichment_jobs_access') THEN
        CREATE POLICY "optimized_enrichment_jobs_access" ON public.enrichment_jobs
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Encrypted data
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'encrypted_data' AND policyname = 'optimized_encrypted_data_access') THEN
        CREATE POLICY "optimized_encrypted_data_access" ON public.encrypted_data
          FOR ALL USING (public.current_user_id() = user_id);
    END IF;

    -- Profiles
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'profiles' AND policyname = 'optimized_profiles_access') THEN
        CREATE POLICY "optimized_profiles_access" ON public.profiles
          FOR ALL USING (public.current_user_id() = id);
    END IF;

    -- User sessions
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'user_sessions' AND policyname = 'optimized_user_sessions_access') THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'interview_transcripts' AND policyname = 'optimized_interview_transcripts_access') THEN
    ...
    -- Resume versions
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'resume_versions' AND policyname = 'optimized_resume_versions_access') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'parsed_resume_entities' AND policyname = 'optimized_parsed_resume_entities_access') THEN
    ...
    -- Resume diffs
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'resume_diffs' AND policyname = 'optimized_resume_diffs_access') THEN
    ...
    -- Job logs
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'job_logs' AND policyname = 'optimized_job_logs_access') THEN
    ...
    -- Normalization jobs
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'normalization_jobs' AND policyname = 'optimized_normalization_jobs_access') THEN
...
-- Step 5: Create policies for remaining tables
DO $$
BEGIN
    -- Processing telemetry
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'processing_telemetry' AND policyname = 'optimized_processing_telemetry_access') THEN
        CREATE POLICY "optimized_processing_telemetry_access" ON public.processing_telemetry
          FOR ALL USING (
            resume_version_id IN (
              SELECT rv.id FROM public.resume_versions rv
              JOIN public.resume_streams rs ON rv.stream_id = rs.id
    ...
    -- Job prompt usage
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'job_prompt_usage' AND policyname = 'optimized_job_prompt_usage_access') THEN
        CREATE POLICY "optimized_job_prompt_usage_access" ON public.job_prompt_usage
          FOR ALL USING (
            job_id IN (
    ...
    -- Resume entity links
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'resume_entity_links' AND policyname = 'optimized_resume_entity_links_access') THEN
        CREATE POLICY "optimized_resume_entity_links_access" ON public.resume_entity_links
          FOR ALL USING (
            parsed_entity_id IN (
              SELECT pre.id FROM public.parsed_resume_entities pre
...
-- Step 6: Create admin-only and public access policies
DO $$
BEGIN
    -- Prompt templates
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'prompt_templates' AND policyname = 'optimized_prompt_templates_admin_access') THEN
        CREATE POLICY "optimized_prompt_templates_admin_access" ON public.prompt_templates
          FOR ALL USING (public.is_admin_user_optimized());
    END IF;

    -- Question flows read access
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'question_flows' AND policyname = 'optimized_question_flows_read_access') THEN
        CREATE POLICY "optimized_question_flows_read_access" ON public.question_flows
          FOR SELECT USING (public.is_authenticated());
    END IF;

    -- Question flows write access
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'question_flows' AND policyname = 'optimized_question_flows_write_access') THEN
        CREATE POLICY "optimized_question_flows_write_access" ON public.question_flows
          FOR INSERT WITH CHECK (public.is_admin_user_optimized());
    END IF;

    -- Question flows update access
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'question_flows' AND policyname = 'optimized_question_flows_update_access') THEN
        CREATE POLICY "optimized_question_flows_update_access" ON public.question_flows
          FOR UPDATE USING (public.is_admin_user_optimized());
    END IF;

    -- Question flows delete access
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'question_flows' AND policyname = 'optimized_question_flows_delete_access') THEN
        CREATE POLICY "optimized_question_flows_delete_access" ON public.question_flows
          FOR DELETE USING (public.is_admin_user_optimized());
    END IF;

    -- Auth rate limits (system only)
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'auth_rate_limits' AND policyname = 'optimized_auth_rate_limits_system_access') THEN
        CREATE POLICY "optimized_auth_rate_limits_system_access" ON public.auth_rate_limits
          FOR ALL USING (false);
    END IF;

    -- Security audit log
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'security_audit_log' AND policyname = 'optimized_security_audit_log_admin_access') THEN
...
-- Step 7: Create supporting indexes for performance
CREATE INDEX IF NOT EXISTS idx_resume_versions_stream_user 
  ON public.resume_versions(stream_id) 
  INCLUDE (id);

CREATE INDEX IF NOT EXISTS idx_resume_streams_user_id 
  ON public.resume_streams(user_id) 
  INCLUDE (id);

CREATE INDEX IF NOT EXISTS idx_parsed_entities_version 
  ON public.parsed_resume_entities(resume_version_id) 
  INCLUDE (id);

CREATE INDEX IF NOT EXISTS idx_job_logs_job_id 
  ON public.job_logs(job_id);

-- Step 8: Update the user deletion function to use optimized patterns
...
-- Step 9: Log the optimization completion
DO $$
BEGIN
...
BEGIN
    ...
    RAISE NOTICE 'Successfully optimized % RLS policies across % tables', 30, 25;
    RAISE NOTICE '';
